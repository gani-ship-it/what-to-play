...
import { Star, Flame, Clock, ChevronRight, Sparkles } from 'lucide-react';
...
export const GameCard: React.FC<GameCardProps> = ({ game, onSelect }) => {
  ...
  const logoUrl = game.steam_appid
  ...
  return (
    <div
      ...
      className="group relative bg-[#12121a] hover:bg-[#181824] border border-white/10 hover:border-[#E50914] rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_15px_30px_rgba(229,9,20,0.25)] cursor-pointer flex flex-col justify-between"
    >
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-zinc-950">
        ...
        {/* Official Game Logo PNG Overlay (Centered over background artwork) */}
        {logoUrl && !logoError && (
          <div className="absolute inset-0 flex items-center justify-center p-4 pointer-events-none z-10">
            <img
              ...
              className="max-h-16 max-w-[75%] object-contain filter drop-shadow-[0_6px_16px_rgba(0,0,0,0.95)] transform group-hover:scale-110 transition-transform duration-500"
            />
          </div>
        )}
        ...
        {/* Top Badges Layer */}
        <div className="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between pointer-events-none gap-2 z-20">
          {/* Status Badges */}
          <div className="flex items-center gap-1.5 flex-wrap">
            {game.is_popular && (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-[#E50914] text-white font-extrabold text-[10px] uppercase tracking-wider shadow-[0_0_10px_rgba(229,9,20,0.6)] backdrop-blur-md">
                <Flame className="w-3 h-3 fill-white" />
                Popular
              </span>
            )}
            {game.is_anticipated && (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-extrabold text-[10px] uppercase tracking-wider shadow-md backdrop-blur-md">
          ...
          {game.rating > 0 && (
            <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-black/85 border border-amber-500/40 text-amber-300 font-extrabold text-[11px] backdrop-blur-md shadow-md">
        ...
        <div className="absolute bottom-2.5 left-2.5 right-2.5 flex items-center justify-between pointer-events-none gap-2 z-20">
          {/* Sentiment Badge */}
          <span className={`px-2 py-0.5 rounded-md border text-[10px] font-bold backdrop-blur-md shadow-md ${sentiment.badgeClass}`}>
            {scorePercent}% Positive
          </span>

          {game.metacritic && (
            <span className="px-2 py-0.5 rounded-md bg-black/85 border border-white/20 text-white text-[10px] font-black font-mono shadow-md backdrop-blur-md">
      ...
      <div className="p-3.5 space-y-2.5 bg-[#12121a] flex-1 flex flex-col justify-between border-t border-white/5">
        <div className="space-y-1.5">
          ...
          {/* Title */}
          <h3 className="font-extrabold text-sm sm:text-base text-white group-hover:text-red-400 transition-colors line-clamp-1 font-heading tracking-tight">
            {game.title}
          </h3>
        </div>
        ...
        <div className="pt-2 border-t border-white/5 flex items-center justify-between text-xs text-zinc-400 font-medium">
          <span className="text-zinc-500 text-[11px]">Release: <strong className="text-zinc-300">{releaseYear}</strong></span>
          <span className="flex items-center gap-1 text-zinc-300 group-hover:text-white transition-colors font-bold text-[10px] uppercase tracking-wider">
            <span>Explore</span>
